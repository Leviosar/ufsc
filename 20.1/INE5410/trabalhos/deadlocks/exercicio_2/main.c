...
void transferir(conta_t *origem, conta_t *destino, double valor) {
    // Como os recursos usados pela thread já são convenientemente numerados
    // pelos identificadores únicos globais das contas, é possível (e fácil)
    // eliminar as condições de espera circular, fazendo com que a thread
    // acesse os mutexes em uma ordem crescente
